...
export const getUserBalances = query({
    handler: async (ctx) => {
        ...
        // Filter expenses to only include one on one expenses (notgroup expenses)
        ...
        /* tallies */
        let youOwe = 0; // amount the user owes to others
        let youAreOwed = 0; // amount others owe to the user
        ...
        for (const e of expenses) {
            ...
            if (isPayer) {
                for (const s of e.splits) {
                    if (s.userId === user._id) continue; // Skip self
                    youAreOwed += s.amount;
        ...
        for (const s of settlements) {
            if (s.paidByUserId === user._id) {
                ...
                // someone paid the user -> reduces what they owed the user 
                youAreOwed -= s.amount;
